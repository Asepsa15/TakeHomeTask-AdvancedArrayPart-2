...
console.log('Array baru dengan data - data berikut:', newData);
console.log(`---------------------------------`);
// Soal No 2 kelompokkan data transaksi berdasarkan lokasi toko
const dataTransaksi = newData.reduce((result, transaction) => {
    const locationIndex = result.findIndex(item => item.location === transaction.storeLocation)
    if (locationIndex === -1) {
        result.push({
        ...
        result[locationIndex].newData.push(transaction)
    }
    return result
}, [])
console.log('Kelompokkan data transaksi berdasarkan lokasi toko:', dataTransaksi);
console.log(`---------------------------------`);
// Soal No 3 hitung keuntungan dari setiap transaksi
const keuntunganTransaksi = newData.map(transaction => {
    const totalProfit = transaction.items.reduce((result, item) => {
...
console.log('Menyimpan keuntungan yang didapat dari setiap transaksi:', keuntunganTransaksi);
console.log(`---------------------------------`);
// Soal No 4 hitung jumlah barang yang dibeli pada setiap transaksi
const jumlahBarangTransaksi = newData.map(transaction => {
    const totalQuantity = transaction.items.reduce((result, item) => {
...
console.log('Menyimpan jumlah barang yang dibeli pada setiap transaksi:', jumlahBarangTransaksi);
console.log(`---------------------------------`);
// Soal No 5 hitung rating setiap toko dari satisfaction customer mereka
const ratingToko = data.reduce((result, transaction) => {
    const locationIndex = result.findIndex(
        (item) => item.location === transaction.storeLocation
    );
    if (locationIndex !== -1) {
        result[locationIndex].ratings.push(transaction.customer.satisfaction.$numberInt);
    } else {
        result.push({
            location: transaction.storeLocation,
            ratings: [transaction.customer.satisfaction.$numberInt],
        });
    }
    return result;
}, []).map((item) => ({
    location: item.location,
    rating: (item.ratings.reduce((sum, rating) => sum + rating, 0) / item.ratings.length).toFixed(1),
}));
console.log('Menyimpan rating setiap toko dari satisfaction customer:', ratingToko);
console.log(`---------------------------------`);
// Soal No 6 hitung keuntungan yang didapat dari setiap lokasi toko
const pendapatanToko = dataTransaksi.map(store => {
    const totalPendapatan = store.newData.reduce((result, transaction) => {
        return result + transaction.items.reduce((sum, item) => {
            return sum + (item.quantity * item.price)
        }, 0)
    }, 0)
    return {
        location: store.location,
        revenue: `USD ${totalPendapatan.toFixed(2)}`
    }
})
console.log('Menyimpan keuntungan yang didapat dari setiap lokasi toko:', pendapatanToko);
console.log(`---------------------------------`);
// Soal No 7 tentukan transaksi dengan omzet terbesar dan terkecil, serta total omzet keseluruhan
const result = data.reduce((acc, transaction) => {
    const totalTransaction = transaction.items.reduce((total, item) => {
        return total + (item.price.$numberDecimal * item.quantity.$numberInt);
    }, 0);

    // mencari transaksi dengan omzet terbesar
    if (totalTransaction > acc.maxRevenue) {
        acc.maxRevenue = totalTransaction;
        acc.maxRevenueTransaction = transaction;
    }

    // mencari transaksi dengan omzet terkecil
    if (totalTransaction < acc.minRevenue || !acc.minRevenue) {
        acc.minRevenue = totalTransaction;
        acc.minRevenueTransaction = transaction;
    }

    acc.totalRevenue += totalTransaction;

    return acc;
}, { maxRevenue: 0, maxRevenueTransaction: null, minRevenue: null, minRevenueTransaction: null, totalRevenue: 0 });
console.log(`Transaksi dengan omzet terbesar:`);
console.log(result.maxRevenueTransaction);
console.log(`Total omzet: ${result.maxRevenue}`);
console.log(`---------------------------------`);
console.log(`Transaksi dengan omzet terkecil:`);
console.log(result.minRevenueTransaction);
console.log(`Total omzet: ${result.minRevenue}`);
console.log(`---------------------------------`);
console.log(`Total omzet dari keseluruhan transaksi: ${result.totalRevenue}`);